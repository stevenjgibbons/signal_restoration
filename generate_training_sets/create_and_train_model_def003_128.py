#
# create_and_train_model_def003_128.py
# Needs to be supplied with a string indicating the stem of the 
# segment definition file, e.g. set00001_128_40000
# Needs to be provided with an integer with the segment length, e.g. 128
# Please hard code "modeldefIDcode =" below with the name of this model
# For this script, it is def003_128.
# If we change the model specifications, then we should make a new script
# with "_def003_128" replaced with something else.
# The output will be saved under "models/" + segdefname + "_def003_128"
# so that we can be sure to separate models trained with different
# models or loss functions.
#
# Added Gaussian smoothing and a change to the loss function to try
# to make the fidelity to the unclipped segments higher.
#
...
# Parameters
number_of_consecutive_samples = 128
...
def custom_loss(y_true, y_pred, status):
    ...
    # Emphasize accuracy where status == 0
    unclipped_mask = tf.cast(tf.equal(status, 0), tf.float32)
    mse_unclipped = tf.square(y_true - y_pred) * unclipped_mask
    unclipped_loss = tf.reduce_sum(mse_unclipped) / (tf.reduce_sum(unclipped_mask) + 1e-6)

    total_loss = tf.reduce_mean(mse + underestimation_penalty) + 0.1 * smoothness_loss + 2.0 * unclipped_loss
    return total_loss
